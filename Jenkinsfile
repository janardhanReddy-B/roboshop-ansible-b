pipeline {
  ...
  stages {
    ...
    stage('Deploy Application') {
      steps {
        sh 'ansible-playbook -i inv roboshop.yml  -e role_name=${COMPONENT} -e env=${ENV} -e app_version=${APP_VERSION} -e ansible_user=centos -e ansible_password=DevOps321 -e "ansible_ssh_timeout=30"'
      }
    }
